...
class Show(db.Model):
    ...
    def past_shows(self): 
      now = datetime.now() 
      past_shows = [x for x in self.shows if datetime.strptime( x.start_time, '%Y-%m-%d %H:%M:%S') < now] 
      return past_shows
...
def show_venue(venue_id):
  ...
  query_venues = Venue.query.get(venue_id)
  data = Venue.venue_detail(query_venues)
...
def show_artist(artist_id):
  ...
  query_artists = Artist.query.get(artist_id)
  data = Artist.artist_detail(query_artists)
